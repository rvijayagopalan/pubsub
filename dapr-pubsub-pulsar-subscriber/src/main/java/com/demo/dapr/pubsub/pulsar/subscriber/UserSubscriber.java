...
public class UserSubscriber {
	...
	@Topic(name = "test-topic", pubsubName = "pulsar-pubsub")
	@PostMapping(path = "/user", consumes = MediaType.ALL_VALUE)
	public Mono<ResponseEntity> subscribe(@RequestBody(required = false) CloudEvent<User> event) {

		return Mono.fromSupplier(() -> {
			try {
				System.out.println("Subscriber received: " + event.getData());
				System.out.println("Subscriber Name : " + event.getData().getName());
